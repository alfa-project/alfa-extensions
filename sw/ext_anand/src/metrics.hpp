...
std::vector<int> ground_class = {ROAD, PARKING, SIDEWALK, OTHER_GROUND,
                                 LANE_MARKING};
std::vector<int> ground_class_with_terrain = {
    ROAD, PARKING, SIDEWALK, OTHER_GROUND, LANE_MARKING, TERRAIN};
std::vector<int> ground_class_with_terrain_and_veg = {
    ROAD, PARKING, SIDEWALK, OTHER_GROUND, LANE_MARKING, TERRAIN, VEGETATION};
std::vector<int> traversable_ground_class = {ROAD, PARKING, LANE_MARKING,
                                             OTHER_GROUND};

std::vector<int> structure_class = {BUILDING, OTHER_STRUCTURE};

std::vector<int> vehicle_class = {
    CAR,        MOVING_CAR,   BICYCLE,       BUS,
...
std::vector<int> human_class = {PERSON,       MOVING_PERSON,
                                BICYCLIST,    MOVING_BICYCLIST,
                                MOTORCYCLIST, MOVING_MOTORCYCLIST};
...
std::vector<int> paris_lille_ground_class = {
    PL_GROUND, PL_ROAD,       PL_SIDEWALK,    PL_CURB,
    PL_ISLAND, PL_VEGETATION, PL_OTHER_GROUND};
...
class Metrics {
 public:
  ...
  Metrics(int no_user_defined = 0, string user_defined_1 = "",
          string user_defined_2 = "", string user_defined_3 = "",
          string user_defined_4 = "", string user_defined_5 = "",
          string user_defined_6 = "");

  ~Metrics();

  void calculate_metrics(pcl::PointCloud<PointT> cloud_in,
                         pcl::PointCloud<PointT> cloud_out, float user_1 = 0,
                         float user_2 = 0, float user_3 = 0, float user_4 = 0,
                         float user_5 = 0, float user_6 = 0);

  void calculate_metrics(pcl::PointCloud<PointT> cloud_in,
                         pcl::PointCloud<PointT> cloud_out,
                         vector<point> vector_out, float user_1 = 0,
                         float user_2 = 0, float user_3 = 0, float user_4 = 0,
                         float user_5 = 0, float user_6 = 0);
 ...
 private:
  ...
  float std_dev_vector(vector<float> v, double vmean) {
    return std::sqrt(std::accumulate(v.begin(), v.end(), 0.0,
                                     [&](double acc, double x) {
                                       return acc + std::pow(x - vmean, 2);
                                     }) /
                     v.size());
  }

  int count_num_ground(const pcl::PointCloud<PointT> &pc);
  int count_num_ground(const pcl::PointCloud<PointT> &input_cloud,
                       const pcl::PointCloud<PointT> &output_cloud,
                       const vector<point> &output_vector, int &TP, int &FP,
                       int &TN, int &FN);
  int count_num_ground(const pcl::PointCloud<PointT> &input_cloud,
                       const pcl::PointCloud<PointT> &output_cloud, int &TP,
                       int &FP, int &TN, int &FN);
  int count_num_ground_rgb(const pcl::PointCloud<PointT> &input_cloud,
                           const pcl::PointCloud<PointT> &output_cloud, int &TP,
                           int &FP, int &TN, int &FN);
  int count_num_ground_label(const pcl::PointCloud<PointT> &input_cloud,
                             const pcl::PointCloud<PointT> &output_cloud,
  ...
  std::vector<pair<int, int>> count_num_each_class(
      const pcl::PointCloud<PointT> pc);
  std::vector<pair<int, int>> set_initial_gt_counts(
...
template <typename PointT>
inline Metrics<PointT>::Metrics(int no_usr_defined, string user_name_1,
                                string user_name_2, string user_name_3,
                                string user_name_4, string user_name_5,
                                string user_name_6) {
  ...
  true_positive_rate.metric_name =
  ...
  positive_predictive_value.metric_name =
...
template <typename PointT>
inline void Metrics<PointT>::calculate_metrics(
    pcl::PointCloud<PointT> cloud_in, pcl::PointCloud<PointT> cloud_out,
    float user_1, float user_2, float user_3, float user_4, float user_5,
    float user_6) {
  int eTP, eFP, eFN, eTN;
  removed_points = count_num_ground(cloud_in, cloud_out, eTP, eFP, eTN,
                                    eFN);  // SemanticKITTI
  // removed_points = count_num_ground_rgb(cloud_in, cloud_out, eTP, eFP, eTN,
  // eFN);  // IITH Varying Slopes removed_points =
  // count_num_ground_label(cloud_in, cloud_out, eTP, eFP, eTN, eFN);  //
  // Paris-Lille-3D
...
inline void Metrics<PointT>::calculate_metrics(
    pcl::PointCloud<PointT> cloud_in, pcl::PointCloud<PointT> cloud_out,
    vector<point> vector_out, float user_1, float user_2, float user_3,
    float user_4, float user_5, float user_6) {
  int eTP, eFP, eFN, eTN;
  removed_points = count_num_ground(cloud_in, cloud_out, vector_out, eTP, eFP,
                                    eTN, eFN);  // SemanticKITTI
...
inline void Metrics<PointT>::callback_shutdown() {
  if (frames > 0) {
    ...
    std::cout << "----------------------" << std::endl;
    std::cout << "TP: " << (int)mean_vector(tp_vec) << " +/- "
              << (int)std_dev_vector(tp_vec, mean_vector(tp_vec)) << std::endl;
    std::cout << "FP: " << (int)mean_vector(fp_vec) << " +/- "
              << (int)std_dev_vector(fp_vec, mean_vector(fp_vec)) << std::endl;
    std::cout << "TN: " << (int)mean_vector(tn_vec) << " +/- "
              << (int)std_dev_vector(tn_vec, mean_vector(tn_vec)) << std::endl;
    std::cout << "FN: " << (int)mean_vector(fn_vec) << " +/- "
              << (int)std_dev_vector(fn_vec, mean_vector(fn_vec)) << std::endl;
    std::cout << "----------------------" << std::endl;
    std::cout.precision(4);
    std::cout << "True Positive Rate: " << _tpr_mean * 100 << " +/- "
              << _tpr_dev * 100 << " %" << std::endl;
    std::cout << "True Negative Rate: " << _tnr_mean * 100 << " +/- "
              << _tnr_dev * 100 << " %" << std::endl;
    std::cout << "Positive Predictive Value: " << _ppv_mean * 100 << " +/- "
              << _ppv_dev * 100 << " %" << std::endl;
    std::cout << "Negative Predictive Value: " << _npv_mean * 100 << " +/- "
              << _npv_dev * 100 << " %" << std::endl;
    std::cout << "F1-Score: " << _f1_mean * 100 << " +/- " << _f1_dev * 100
              << " %" << std::endl;
    std::cout << "Accuracy: " << _acc_mean * 100 << " +/- " << _acc_dev * 100
              << " %" << std::endl;
    std::cout << "IoUg: " << _ioug_mean * 100 << " +/- " << _ioug_dev * 100
              << " %" << std::endl;
    std::cout << "----------------------" << std::endl;
    std::cout.precision(6);
    std::cout << "Total points [In]: " << (int)total_mean << " +/- "
              << (int)total_dev << std::endl;
    std::cout << "Total points [Out]: " << (int)totalo_mean << " +/- "
              << (int)totalo_dev << std::endl;
    std::cout << "Removed points: " << (int)removed_mean << " +/- "
              << (int)removed_dev << std::endl;
    std::cout << "----------------------" << std::endl;
    std::cout << "Points outside of grid: " << (int)usr2_mean << " +/- "
              << (int)usr2_dev << std::endl;
    std::cout << "Removed points (Handler): " << (int)usr1_mean << " +/- "
              << (int)usr1_dev << std::endl;
    std::cout << "----------------------" << std::endl;
    std::cout.precision(4);
    std::cout << "Handler Time: " << ht_mean / 1000 << " +/- " << ht_dev / 1000
              << " ms" << std::endl;
    std::cout << "Full Processing Time: " << ft_mean / 1000 << " +/- "
              << ft_dev / 1000 << " ms" << std::endl;
    std::cout << "-----------------------" << std::endl;

    switch (no_usr_defined_vec) {
      case 6:
        std::cout << user_defined_6.name << ": "
                  << static_cast<int>(
                         std::accumulate(user_defined_vec_6.begin(),
                                         user_defined_vec_6.end(), 0.0) /
                         user_defined_vec_6.size())
                  << "" << std::endl;

      case 5:
        std::cout << user_defined_5.name << ": "
                  << static_cast<int>(
                         std::accumulate(user_defined_vec_5.begin(),
                                         user_defined_vec_5.end(), 0.0) /
                         user_defined_vec_5.size())
                  << "" << std::endl;

      case 4:
        std::cout << user_defined_4.name << ": "
                  << static_cast<int>(
                         std::accumulate(user_defined_vec_4.begin(),
                                         user_defined_vec_4.end(), 0.0) /
                         user_defined_vec_4.size())
                  << "" << std::endl;

      case 3:
        std::cout << user_defined_3.name << ": "
                  << static_cast<int>(
                         std::accumulate(user_defined_vec_3.begin(),
                                         user_defined_vec_3.end(), 0.0) /
                         user_defined_vec_3.size())
                  << "" << std::endl;

      case 2:
        std::cout << user_defined_2.name << ": "
                  << static_cast<int>(
                         std::accumulate(user_defined_vec_2.begin(),
                                         user_defined_vec_2.end(), 0.0) /
                         user_defined_vec_2.size())
                  << "" << std::endl;

      case 1:
        std::cout << user_defined_1.name << ": "
                  << static_cast<int>(
                         std::accumulate(user_defined_vec_1.begin(),
                                         user_defined_vec_1.end(), 0.0) /
                         user_defined_vec_1.size())
                  << "" << std::endl;
        break;
...
template <typename PointT>
inline void Metrics<PointT>::post_processing(
    alfa_msg::msg::MetricMessage handler_time,
    alfa_msg::msg::MetricMessage full_time) {
  frames++;

  std::cout << "\r\e[K" << "Point cloud [" << frames << "] received!!"
            << std::flush;
...
inline void Metrics<PointT>::publish_output_metrics() {
  ...
  switch (no_usr_defined_vec) {
    case 6:
      user_defined_vec_6.push_back(user_defined_6.value);

    case 5:
      user_defined_vec_5.push_back(user_defined_5.value);

    case 4:
      user_defined_vec_4.push_back(user_defined_4.value);

    case 3:
      user_defined_vec_3.push_back(user_defined_3.value);

    case 2:
      user_defined_vec_2.push_back(user_defined_2.value);

    case 1:
      user_defined_vec_1.push_back(user_defined_1.value);
      break;
...
template <typename PointT>
inline void Metrics<PointT>::get_classes_in_cloud(
    pcl::PointCloud<PointT> cloud_in, bool select) {
...
template <typename PointT>
inline int Metrics<PointT>::count_num_ground(
    const pcl::PointCloud<PointT> &input_cloud,
    const pcl::PointCloud<PointT> &output_cloud, int &TP, int &FP, int &TN,
    int &FN) {
  ...
  for (int i = 0; i < input_cloud.size(); i++) {
    iter = std::find(outlier_class.begin(), outlier_class.end(),
                     (std::uint16_t)input_cloud.points[i].custom_field);
    if (iter == outlier_class.end()) {
      iter = std::find(ground_class_with_terrain_and_veg.begin(),
                       ground_class_with_terrain_and_veg.end(),
                       (std::uint16_t)input_cloud.points[i].custom_field);
      if (iter != ground_class_with_terrain_and_veg
                      .end()) {  // corresponding class is in ground classes
...
inline int Metrics<PointT>::count_num_ground(
    const pcl::PointCloud<PointT> &input_cloud,
    const pcl::PointCloud<PointT> &output_cloud,
    const vector<point> &output_vector, int &TP, int &FP, int &TN, int &FN) {
  std::vector<int>::iterator iter;
  ...
  for (const auto &p : output_vector) {
    ...
    iter = std::find(ground_class_with_terrain_and_veg.begin(),
                     ground_class_with_terrain_and_veg.end(),
                     (std::uint16_t)input_cloud.points[id].custom_field);
    if (iter != ground_class_with_terrain_and_veg
                    .end()) {  // corresponding class is in ground classes
...
template <typename PointT>
inline int Metrics<PointT>::count_num_ground_label(
    const pcl::PointCloud<PointT> &input_cloud,
    const pcl::PointCloud<PointT> &output_cloud, int &TP, int &FP, int &TN,
    int &FN) {
  std::vector<int>::iterator iter;
  ...
  for (int i = 0; i < input_cloud.size(); i++) {
    iter = std::find(paris_lille_ground_class.begin(),
                     paris_lille_ground_class.end(),
                     input_cloud.points[i].custom_field);
    if (iter != paris_lille_ground_class
                    .end()) {  // corresponding class is in ground classes
      if (output_cloud.points[i].custom_field > 0) {
        TP++;
        num_removed++;
        ...
        FN++;
        // std::cout << input_cloud.points[i].custom_field << " " <<
        // output_cloud.points[i].custom_field << endl;
      }
    }
...
template <typename PointT>
inline int Metrics<PointT>::count_num_ground_rgb(
    const pcl::PointCloud<PointT> &input_cloud,
    const pcl::PointCloud<PointT> &output_cloud, int &TP, int &FP, int &TN,
    int &FN) {
  std::vector<int>::iterator iter;
...
inline std::vector<pair<int, int>> Metrics<PointT>::count_num_each_class(
  ...
  for (auto const pt : pc.points) {
    iter = std::find(all_classes.begin(), all_classes.end(),
                     (std::uint16_t)pt.custom_field);
...
template <typename PointT>
inline std::pair<int, int> Metrics<PointT>::count_veg(
    const pcl::PointCloud<PointT> &pc) {
  int num_veg = 0;
